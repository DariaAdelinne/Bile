...
fun fetchYesterdayNews(symbol: String): List<NewsArticle> {
    val fmt = DateTimeFormatter.ofPattern("yyyy-MM-dd")
    val today = LocalDate.now()
    val yesterday = today.minusDays(1)
    val dateFrom = yesterday.format(fmt)
...
fun handleClient(client: Socket, symbols: List<String>) {
    ...
    for (symbol in symbols) {
        ...
        println("Preiau stirile pentru: $symbol")
        val articles = fetchYesterdayNews(symbol)
...
fun main() {
    ...
    println("FinnhubNewsTCPServer pornit pe portul $TCP_PORT. Astept conexiuni...")
