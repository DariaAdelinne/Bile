...
class RabbitMqProducer(RabbitMqInterface):
    """
    Producator RabbitMQ — trimite mesaje pe un exchange cu un routing key dat.
...
class RabbitMqConsumer(RabbitMqInterface):
    """
    Consumator RabbitMQ — citeste mesaje dintr-o coada.
    ...
    def __init__(self, rabbit_queue: str):
        ...
        # Goleste coada la initializare (pentru rulare curata)
    ...
    def receive_message(self) -> str:
        ...
        Returneaza None daca nu exista mesaje dupa 15 incercari.
        """
    ...
    def receive_message_infinite_tries(self) -> str:
        """
        Asteapta un mesaj la infinit (folosit de Bidder si LoggerProcessor).
        """
        result_msg = self.channel.basic_get(self.config['queue'])
        if result_msg[2]:
            self.channel.basic_ack(result_msg[0].delivery_tag)
