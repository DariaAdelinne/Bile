...
/**
 * AuctioneerMicroservice - nemodificat semnificativ fata de laboratorul 7.
 * Accepta oferte de la bidderi timp de AUCTION_DURATION ms,
 * le trimite la MessageProcessorMicroservice, asteapta castigatorul
 * de la BiddingProcessorMicroservice si notifica toti bidderii.
 */
class AuctioneerMicroservice {
            ...
            while (true) {
                try {
                    ...
                    if (receivedMessage == null) {
                        ...
                        emitter.onError(Exception("Bidder-ul ${bidderConnection.port} s-a deconectat."))
                    }
    ...
    private fun finishAuction() {
        try {
            ...
            println("Nu ma pot conecta la BiddingProcessor! $e")
            auctioneerSocket.close()
            exitProcess(1)
        }
