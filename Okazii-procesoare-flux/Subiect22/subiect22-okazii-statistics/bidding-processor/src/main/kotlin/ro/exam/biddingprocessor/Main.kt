...
class BiddingProcessorMicroservice(
    ...
    private fun parseBid(body: String): Int =
        body.removePrefix("licitez ").trim().split(Regex("\\s+")).first().toInt()
    ...
    private fun sendWinner(winner: Message?) {
        ...
        statistics.record(StatisticsEventType.RESULT_TO_AUCTIONEER)
